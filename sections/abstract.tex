...
\begin{abstract}
    ...
    On points on an elliptic curve we can define an addition law, which makes the set of points on the elliptic curve into an abelian group with the identity element being the point at infinity.
    ...
    The Mordell-Weil group is an important object in the study of arithmetic of elliptic curves.
    ...
    Using the parameterization of Pythagorean triples $(a,b,c) = (2mn, m^{2} - n^{2}, m^{2} + n^{2})$ by $m,n \in \mathbb{Z}$ with $(m,n)=1$ and putting $s = m/n$, we can one-parameterize Frey curves of degree two, and regard the family as an elliptic curve
    \begin{equation*}
        E_{1,s}: y^{2} = x(x - 4s^{2})(x + (s^{2} - 1)^{2})
    \end{equation*}
    over a function field $\mathbb{Q}(s)$.
    ...
    Also, we construct an infinite subfamily of $E_{1,s}$ whose Mordell-Weil rank over $\overline{\mathbb{Q}}(s)$ is $1$.
